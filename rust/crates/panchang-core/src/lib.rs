...
pub use crate::day_segments::{civil_day, month};
...
pub fn snapshot(req: SnapshotRequest) -> Result<SnapshotResponse, PanchangError> {
    ...
    let sunset_jd_ut = sunrise_jd_ut.and_then(|sr| rise_set::find_next_sunset_jd(sr + 1e-6, req.latitude, req.longitude));
    let next_sunrise_jd_ut = sunset_jd_ut.and_then(|ss| rise_set::find_next_sunrise_jd(ss + 1e-6, req.latitude, req.longitude));
    let hora = match (sunrise_jd_ut, sunset_jd_ut, next_sunrise_jd_ut) {
        (Some(sr), Some(ss), Some(ns)) => hora::build_hora_table(sr, ss, ns, tz),
        _ => Vec::new(),
    };
    let current_hora = hora.iter().find(|h| h.jd_start <= jd && jd < h.jd_end).cloned();
    ...
    Ok(SnapshotResponse {
        ...
        moon_sidereal_deg,
        angas,
        vaara_civil_local: names::weekday_name(utc.with_timezone(&tz).weekday().num_days_from_monday() as usize).to_string(),
        sunrise_jd_ut,
        sunset_jd_ut,
        next_sunrise_jd_ut,

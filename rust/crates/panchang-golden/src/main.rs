...
fn main() -> anyhow::Result<()> {
    ...
    let raw = std::fs::read_to_string(&fixture_path).with_context(|| format!("read {fixture_path}"))?;
    let fixtures: Vec<SnapshotRequest> = serde_json::from_str(&raw).context("parse fixture requests")?;
    for req in fixtures {
